...
void StopCMS3::Init(TTree *tree) {
  ...
  MT2_had_branch = tree->GetBranch("MT2_had");
  if (MT2_had_branch) MT2_had_branch->SetAddress(&MT2_had_);
  MT2_had_genmet_branch = tree->GetBranch("MT2_had_genmet");
  if (MT2_had_genmet_branch) MT2_had_genmet_branch->SetAddress(&MT2_had_genmet_);
  MT2_had_jup_branch = tree->GetBranch("MT2_had_jup");
  if (MT2_had_jup_branch) MT2_had_jup_branch->SetAddress(&MT2_had_jup_);
  MT2_had_jdown_branch = tree->GetBranch("MT2_had_jdown");
  if (MT2_had_jdown_branch) MT2_had_jdown_branch->SetAddress(&MT2_had_jdown_);
  mht_pt_branch = tree->GetBranch("mht_pt");
  if (mht_pt_branch) mht_pt_branch->SetAddress(&mht_pt_);
  mht_pt_jup_branch = tree->GetBranch("mht_pt_jup");
  if (mht_pt_jup_branch) mht_pt_jup_branch->SetAddress(&mht_pt_jup_);
  mht_pt_jdown_branch = tree->GetBranch("mht_pt_jdown");
  if (mht_pt_jdown_branch) mht_pt_jdown_branch->SetAddress(&mht_pt_jdown_);
  mht_phi_branch = tree->GetBranch("mht_phi");
  if (mht_phi_branch) mht_phi_branch->SetAddress(&mht_phi_);
  mht_phi_jup_branch = tree->GetBranch("mht_phi_jup");
  if (mht_phi_jup_branch) mht_phi_jup_branch->SetAddress(&mht_phi_jup_);
  mht_phi_jdown_branch = tree->GetBranch("mht_phi_jdown");
  if (mht_phi_jdown_branch) mht_phi_jdown_branch->SetAddress(&mht_phi_jdown_);
...
void StopCMS3::GetEntry(unsigned int idx) {
  ...
  MT2_had_isLoaded = false;
  MT2_had_genmet_isLoaded = false;
  MT2_had_jup_isLoaded = false;
  MT2_had_jdown_isLoaded = false;
  mht_pt_isLoaded = false;
  mht_pt_jup_isLoaded = false;
  mht_pt_jdown_isLoaded = false;
  mht_phi_isLoaded = false;
  mht_phi_jup_isLoaded = false;
  mht_phi_jdown_isLoaded = false;
...
void StopCMS3::LoadAllBranches() {
  ...
  if (MT2_had_branch != 0) MT2_had();
  if (MT2_had_genmet_branch != 0) MT2_had_genmet();
  if (MT2_had_jup_branch != 0) MT2_had_jup();
  if (MT2_had_jdown_branch != 0) MT2_had_jdown();
  if (mht_pt_branch != 0) mht_pt();
  if (mht_pt_jup_branch != 0) mht_pt_jup();
  if (mht_pt_jdown_branch != 0) mht_pt_jdown();
  if (mht_phi_branch != 0) mht_phi();
  if (mht_phi_jup_branch != 0) mht_phi_jup();
  if (mht_phi_jdown_branch != 0) mht_phi_jdown();
...
const float &StopCMS3::pfmet_jup() {
...
const float &StopCMS3::pfmet_phi_jup() {
...
const float &StopCMS3::pfmet_jdown() {
...
const float &StopCMS3::pfmet_phi_jdown() {
...
const float &StopCMS3::pfmet_rl_jup() {
...
const float &StopCMS3::pfmet_phi_rl_jup() {
...
const float &StopCMS3::pfmet_rl_jdown() {
...
const float &StopCMS3::pfmet_phi_rl_jdown() {
...
const float &StopCMS3::pfmet_uncorr() {
...
const float &StopCMS3::pfmet_uncorr_phi() {
...
const float &StopCMS3::pfmet_original() {
...
const float &StopCMS3::pfmet_original_phi() {
...
const float &StopCMS3::MT2W_rl_jup() {
...
const float &StopCMS3::MT2_ll_jup() {
...
const float &StopCMS3::mindphi_met_j1_j2_jup() {
...
const float &StopCMS3::mindphi_met_j1_j2_rl_jup() {
...
const float &StopCMS3::mt_met_lep_jup() {
...
const float &StopCMS3::mt_met_lep_rl_jup() {
...
const float &StopCMS3::MT2W_jdown() {
...
const float &StopCMS3::MT2W_rl_jdown() {
...
const float &StopCMS3::MT2_ll_jdown() {
...
const float &StopCMS3::mindphi_met_j1_j2_jdown() {
...
const float &StopCMS3::mindphi_met_j1_j2_rl_jdown() {
...
const float &StopCMS3::mt_met_lep_jdown() {
...
const float &StopCMS3::mt_met_lep_rl_jdown() {
...
const int &StopCMS3::ngenweights() {
...
const float &StopCMS3::weight_Q2_up() {
...
const float &StopCMS3::weight_Q2_down() {
...
const float &StopCMS3::weight_muF_up() {
...
const float &StopCMS3::weight_muF_down() {
...
const float &StopCMS3::weight_muR_up() {
...
const float &StopCMS3::weight_muR_down() {
...
const float &StopCMS3::weight_alphas_up() {
...
const float &StopCMS3::weight_alphas_down() {
...
const float &StopCMS3::weight_analysisbtagsf() {
...
const float &StopCMS3::weight_analysisbtagsf_heavy_UP() {
...
const float &StopCMS3::weight_analysisbtagsf_light_UP() {
...
const float &StopCMS3::weight_analysisbtagsf_heavy_DN() {
...
const float &StopCMS3::weight_analysisbtagsf_light_DN() {
...
const float &StopCMS3::weight_analysisbtagsf_fastsim_UP() {
...
const float &StopCMS3::weight_analysisbtagsf_fastsim_DN() {
...
const float &StopCMS3::weight_tightbtagsf() {
...
const float &StopCMS3::weight_tightbtagsf_heavy_UP() {
...
const float &StopCMS3::weight_tightbtagsf_light_UP() {
...
const float &StopCMS3::weight_tightbtagsf_heavy_DN() {
...
const float &StopCMS3::weight_tightbtagsf_light_DN() {
...
const float &StopCMS3::weight_tightbtagsf_fastsim_UP() {
...
const float &StopCMS3::weight_tightbtagsf_fastsim_DN() {
...
const float &StopCMS3::weight_loosebtagsf() {
...
const float &StopCMS3::weight_loosebtagsf_heavy_UP() {
...
const float &StopCMS3::weight_loosebtagsf_light_UP() {
...
const float &StopCMS3::weight_loosebtagsf_heavy_DN() {
...
const float &StopCMS3::weight_loosebtagsf_light_DN() {
...
const float &StopCMS3::weight_loosebtagsf_fastsim_UP() {
...
const float &StopCMS3::weight_loosebtagsf_fastsim_DN() {
...
const float &StopCMS3::weight_L1prefire() {
...
const float &StopCMS3::weight_L1prefire_UP() {
...
const float &StopCMS3::weight_L1prefire_DN() {
...
const float &StopCMS3::topnessMod_jup() {
...
const float &StopCMS3::topnessMod_rl_jup() {
...
const float &StopCMS3::topnessMod_jdown() {
...
const float &StopCMS3::topnessMod_rl_jdown() {
...
const float &StopCMS3::Mlb_closestb_jup() {
...
const float &StopCMS3::Mlb_lead_bdiscr_jup() {
...
const float &StopCMS3::Mlb_closestb_jdown() {
...
const float &StopCMS3::Mlb_lead_bdiscr_jdown() {
...
const int &StopCMS3::HLT_MET_MHT() {
...
const int &StopCMS3::HLT_MET110_MHT110() {
...
const int &StopCMS3::HLT_MET120_MHT120() {
...
const int &StopCMS3::HLT_MET130_MHT130() {
...
const int &StopCMS3::HLT_PFHT_unprescaled() {
...
const int &StopCMS3::HLT_PFHT_prescaled() {
...
const int &StopCMS3::HLT_AK8Jet_unprescaled() {
...
const int &StopCMS3::HLT_AK8Jet_prescaled() {
...
const int &StopCMS3::HLT_CaloJet500_NoJetID() {
...
const float &StopCMS3::lep1_etaSC() {
...
const float &StopCMS3::lep1_dphiMET() {
...
const float &StopCMS3::lep1_dphiMET_jup() {
...
const float &StopCMS3::lep1_dphiMET_jdown() {
...
const float &StopCMS3::lep1_dphiMET_rl() {
...
const float &StopCMS3::lep1_dphiMET_rl_jup() {
...
const float &StopCMS3::lep1_dphiMET_rl_jdown() {
...
const float &StopCMS3::lep1_dphiMET_resup() {
...
const float &StopCMS3::lep1_dphiMET_resdown() {
...
const float &StopCMS3::lep1_dphiMET_rl_resup() {
...
const float &StopCMS3::lep1_dphiMET_rl_resdown() {
...
const float &StopCMS3::lep1_dphiGenMET() {
...
const float &StopCMS3::lep1_dphiGenMET_rl() {
...
const float &StopCMS3::lep2_etaSC() {
...
const float &StopCMS3::lep2_dphiMET() {
...
const float &StopCMS3::lep2_dphiMET_jup() {
...
const float &StopCMS3::lep2_dphiMET_jdown() {
...
const float &StopCMS3::lep2_dphiMET_rl() {
...
const float &StopCMS3::lep2_dphiMET_rl_jup() {
...
const float &StopCMS3::lep2_dphiMET_rl_jdown() {
...
const float &StopCMS3::lep2_dphiMET_resup() {
...
const float &StopCMS3::lep2_dphiMET_resdown() {
...
const float &StopCMS3::lep2_dphiMET_rl_resup() {
...
const float &StopCMS3::lep2_dphiMET_rl_resdown() {
...
const float &StopCMS3::lep2_dphiGenMET() {
...
const float &StopCMS3::lep2_dphiGenMET_rl() {
...
const vector<bool> &StopCMS3::ph_passLooseID() {
...
const vector<bool> &StopCMS3::ph_passMediumID() {
...
const vector<bool> &StopCMS3::ph_passTightID() {
...
const int &StopCMS3::nskimbtagmed() {
...
const int &StopCMS3::nskimbtagloose() {
...
const int &StopCMS3::nskimbtagtight() {
...
const int &StopCMS3::nloosebtags() {
...
const int &StopCMS3::ntightbtags() {
...
const int &StopCMS3::nanalysisbtags() {
...
const float &StopCMS3::ak4_HTeta5() {
...
const float &StopCMS3::ak4_MHT_pt() {
...
const float &StopCMS3::ak4_MHT_phi() {
...
const vector<float> &StopCMS3::ak4pfjets_deepCSV() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::softtags_p4() {
...
const int &StopCMS3::jup_nskimjets() {
...
const int &StopCMS3::jup_nskimbtagmed() {
...
const int &StopCMS3::jup_nskimbtagloose() {
...
const int &StopCMS3::jup_nskimbtagtight() {
...
const int &StopCMS3::jup_ngoodjets() {
...
const int &StopCMS3::jup_ngoodbtags() {
...
const int &StopCMS3::jup_nloosebtags() {
...
const int &StopCMS3::jup_ntightbtags() {
...
const int &StopCMS3::jup_nanalysisbtags() {
...
const int &StopCMS3::jup_nfailjets() {
...
const float &StopCMS3::jup_ak4_HT() {
...
const float &StopCMS3::jup_ak4_HTeta5() {
...
const float &StopCMS3::jup_ak4_MHT_pt() {
...
const float &StopCMS3::jup_ak4_MHT_phi() {
...
const float &StopCMS3::jup_ak4_htratiom() {
...
const vector<float> &StopCMS3::jup_dphi_ak4pfjet_met() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jup_ak4pfjets_p4() {
...
const vector<bool> &StopCMS3::jup_ak4pfjets_passMEDbtag() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_deepCSV() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_CSV() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_mva() {
...
const vector<int> &StopCMS3::jup_ak4pfjets_parton_flavor() {
...
const vector<int> &StopCMS3::jup_ak4pfjets_hadron_flavor() {
...
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &StopCMS3::jup_ak4pfjets_leadMEDbjet_p4() {
...
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &StopCMS3::jup_ak4pfjets_leadbtag_p4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jup_ak4genjets_p4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jup_softtags_p4() {
...
const int &StopCMS3::jup_nsoftbtags() {
...
const int &StopCMS3::jdown_nskimjets() {
...
const int &StopCMS3::jdown_nskimbtagmed() {
...
const int &StopCMS3::jdown_nskimbtagloose() {
...
const int &StopCMS3::jdown_nskimbtagtight() {
...
const int &StopCMS3::jdown_ngoodjets() {
...
const int &StopCMS3::jdown_ngoodbtags() {
...
const int &StopCMS3::jdown_nloosebtags() {
...
const int &StopCMS3::jdown_ntightbtags() {
...
const int &StopCMS3::jdown_nanalysisbtags() {
...
const int &StopCMS3::jdown_nfailjets() {
...
const float &StopCMS3::jdown_ak4_HT() {
...
const float &StopCMS3::jdown_ak4_HTeta5() {
...
const float &StopCMS3::jdown_ak4_MHT_pt() {
...
const float &StopCMS3::jdown_ak4_MHT_phi() {
...
const float &StopCMS3::jdown_ak4_htratiom() {
...
const vector<float> &StopCMS3::jdown_dphi_ak4pfjet_met() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jdown_ak4pfjets_p4() {
...
const vector<bool> &StopCMS3::jdown_ak4pfjets_passMEDbtag() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_deepCSV() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_CSV() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_mva() {
...
const vector<int> &StopCMS3::jdown_ak4pfjets_parton_flavor() {
...
const vector<int> &StopCMS3::jdown_ak4pfjets_hadron_flavor() {
...
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &StopCMS3::jdown_ak4pfjets_leadMEDbjet_p4() {
...
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &StopCMS3::jdown_ak4pfjets_leadbtag_p4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jdown_ak4genjets_p4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jdown_softtags_p4() {
...
const int &StopCMS3::jdown_nsoftbtags() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::ak8pfjets_p4() {
...
const vector<float> &StopCMS3::ak8pfjets_tau1() {
...
const vector<float> &StopCMS3::ak8pfjets_tau2() {
...
const vector<float> &StopCMS3::ak8pfjets_tau3() {
...
const vector<int> &StopCMS3::ak8pfjets_parton_flavor() {
...
const int &StopCMS3::nGoodAK8PFJets() {
...
const vector<float> &StopCMS3::ak8pfjets_deepdisc_qcd() {
...
const vector<float> &StopCMS3::ak8pfjets_deepdisc_top() {
...
const vector<float> &StopCMS3::ak8pfjets_deepdisc_w() {
...
const vector<float> &StopCMS3::ak8pfjets_deepdisc_z() {
...
const vector<float> &StopCMS3::ak8pfjets_deepdisc_zbb() {
...
const vector<float> &StopCMS3::ak8pfjets_deepdisc_hbb() {
...
const vector<float> &StopCMS3::ak8pfjets_deepdisc_h4q() {
...
const float &StopCMS3::lead_ak8deepdisc_top() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jup_ak8pfjets_p4() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_tau1() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_tau2() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_tau3() {
...
const vector<int> &StopCMS3::jup_ak8pfjets_parton_flavor() {
...
const int &StopCMS3::jup_nGoodAK8PFJets() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_deepdisc_qcd() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_deepdisc_top() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_deepdisc_w() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_deepdisc_z() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_deepdisc_zbb() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_deepdisc_hbb() {
...
const vector<float> &StopCMS3::jup_ak8pfjets_deepdisc_h4q() {
...
const float &StopCMS3::jup_lead_ak8deepdisc_top() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jdown_ak8pfjets_p4() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_tau1() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_tau2() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_tau3() {
...
const vector<int> &StopCMS3::jdown_ak8pfjets_parton_flavor() {
...
const int &StopCMS3::jdown_nGoodAK8PFJets() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_deepdisc_qcd() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_deepdisc_top() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_deepdisc_w() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_deepdisc_z() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_deepdisc_zbb() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_deepdisc_hbb() {
...
const vector<float> &StopCMS3::jdown_ak8pfjets_deepdisc_h4q() {
...
const float &StopCMS3::jdown_lead_ak8deepdisc_top() {
...
const vector<float> &StopCMS3::ak4pfjets_cvsl() {
...
const vector<float> &StopCMS3::ak4pfjets_ptD() {
...
const vector<float> &StopCMS3::ak4pfjets_axis1() {
...
const vector<float> &StopCMS3::ak4pfjets_axis2() {
...
const vector<int> &StopCMS3::ak4pfjets_mult() {
...
const vector<float> &StopCMS3::ak4pfjets_deepCSVb() {
...
const vector<float> &StopCMS3::ak4pfjets_deepCSVbb() {
...
const vector<float> &StopCMS3::ak4pfjets_deepCSVc() {
...
const vector<float> &StopCMS3::ak4pfjets_deepCSVl() {
...
const vector<vector<int> > &StopCMS3::topcands_ak4idx() {
...
const vector<float> &StopCMS3::topcands_disc() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::topcands_p4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::topcands_Wp4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::tftops_p4() {
...
const vector<float> &StopCMS3::tftops_disc() {
...
const vector<vector<float> > &StopCMS3::tftops_subjet_pt() {
...
const vector<vector<float> > &StopCMS3::tftops_subjet_eta() {
...
const vector<vector<float> > &StopCMS3::tftops_subjet_phi() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_cvsl() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_ptD() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_axis1() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_axis2() {
...
const vector<int> &StopCMS3::jup_ak4pfjets_mult() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_deepCSVb() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_deepCSVbb() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_deepCSVc() {
...
const vector<float> &StopCMS3::jup_ak4pfjets_deepCSVl() {
...
const vector<vector<int> > &StopCMS3::jup_topcands_ak4idx() {
...
const vector<float> &StopCMS3::jup_topcands_disc() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jup_topcands_p4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jup_topcands_Wp4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jup_tftops_p4() {
...
const vector<float> &StopCMS3::jup_tftops_disc() {
...
const vector<vector<float> > &StopCMS3::jup_tftops_subjet_pt() {
...
const vector<vector<float> > &StopCMS3::jup_tftops_subjet_eta() {
...
const vector<vector<float> > &StopCMS3::jup_tftops_subjet_phi() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_cvsl() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_ptD() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_axis1() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_axis2() {
...
const vector<int> &StopCMS3::jdown_ak4pfjets_mult() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_deepCSVb() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_deepCSVbb() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_deepCSVc() {
...
const vector<float> &StopCMS3::jdown_ak4pfjets_deepCSVl() {
...
const vector<vector<int> > &StopCMS3::jdown_topcands_ak4idx() {
...
const vector<float> &StopCMS3::jdown_topcands_disc() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jdown_topcands_p4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jdown_topcands_Wp4() {
...
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &StopCMS3::jdown_tftops_p4() {
...
const vector<float> &StopCMS3::jdown_tftops_disc() {
...
const vector<vector<float> > &StopCMS3::jdown_tftops_subjet_pt() {
...
const vector<vector<float> > &StopCMS3::jdown_tftops_subjet_eta() {
...
const vector<vector<float> > &StopCMS3::jdown_tftops_subjet_phi() {
...
const float &StopCMS3::pfmet_uncorr_jup() {
...
const float &StopCMS3::pfmet_uncorr_phi_jup() {
...
const float &StopCMS3::pfmet_uncorr_jdown() {
...
const float &StopCMS3::pfmet_uncorr_phi_jdown() {
...
const float &StopCMS3::pfmet_resup() {
...
const float &StopCMS3::pfmet_phi_resup() {
...
const float &StopCMS3::pfmet_resdown() {
...
const float &StopCMS3::pfmet_phi_resdown() {
...
const float &StopCMS3::pfmet_rl_resup() {
...
const float &StopCMS3::pfmet_phi_rl_resup() {
...
const float &StopCMS3::pfmet_rl_resdown() {
...
const float &StopCMS3::mt_met_lep_resup() {
...
const float &StopCMS3::mt_met_lep_rl_resup() {
...
const float &StopCMS3::mt_met_lep_resdown() {
...
const float &StopCMS3::mt_met_lep_rl_resdown() {
...
const float &StopCMS3::topnessMod_resup() {
...
const float &StopCMS3::topnessMod_rl_resup() {
...
const float &StopCMS3::topnessMod_resdown() {
...
const float &StopCMS3::topnessMod_rl_resdown() {
...
const float &StopCMS3::mindphi_met_j1_j2_resup() {
...
const float &StopCMS3::mindphi_met_j1_j2_rl_resup() {
...
const float &StopCMS3::mindphi_met_j1_j2_resdown() {
...
const float &StopCMS3::mindphi_met_j1_j2_rl_resdown() {
...
const float &StopCMS3::MT2_ll_resup() {
...
const float &StopCMS3::MT2_ll_resdown() {
...
const float &StopCMS3::genmet_rl() {
...
const float &StopCMS3::genmet_phi_rl() {
...
const float &StopCMS3::topnessMod_genmet() {
...
const float &StopCMS3::topnessMod_rl_genmet() {
...
const float &StopCMS3::mt_genmet_lep() {
...
const float &StopCMS3::mt_genmet_lep_rl() {
...
const float &StopCMS3::mindphi_genmet_j1_j2() {
...
const float &StopCMS3::mindphi_genmet_j1_j2_rl() {
...
const float &StopCMS3::MT2_ll_genmet() {
...
const bool &StopCMS3::filt_cscbeamhalo() {
...
const bool &StopCMS3::filt_cscbeamhalo2015() {
...
const bool &StopCMS3::filt_globaltighthalo2016() {
...
const bool &StopCMS3::filt_globalsupertighthalo2016() {
...
const bool &StopCMS3::filt_ecallaser() {
...
const bool &StopCMS3::filt_ecaltp() {
...
const bool &StopCMS3::filt_ecalbadcalib() {
...
const bool &StopCMS3::filt_eebadsc() {
...
const bool &StopCMS3::filt_goodvtx() {
...
const bool &StopCMS3::filt_badevents() {
...
const bool &StopCMS3::filt_hbhenoise() {
...
const bool &StopCMS3::filt_hbheisonoise() {
...
const bool &StopCMS3::filt_hcallaser() {
...
const bool &StopCMS3::filt_trkfail() {
...
const bool &StopCMS3::filt_trkPOG() {
...
const bool &StopCMS3::filt_trkPOG_logerr_tmc() {
...
const bool &StopCMS3::filt_trkPOG_tmc() {
...
const bool &StopCMS3::filt_trkPOG_tms() {
...
const int &StopCMS3::firstGoodVtxIdx() {
...
const bool &StopCMS3::filt_badChargedCandidateFilter() {
...
const bool &StopCMS3::filt_badMuonFilter() {
...
const bool &StopCMS3::filt_fastsimjets_jup() {
...
const bool &StopCMS3::filt_fastsimjets_jdown() {
...
const bool &StopCMS3::filt_jetWithBadMuon() {
...
const bool &StopCMS3::filt_jetWithBadMuon_jup() {
...
const bool &StopCMS3::filt_jetWithBadMuon_jdown() {
...
const bool &StopCMS3::filt_pfovercalomet() {
...
const bool &StopCMS3::filt_badmuons() {
...
const bool &StopCMS3::filt_duplicatemuons() {
...
const bool &StopCMS3::filt_nobadmuons() {
...
const float &StopCMS3::MT2_had() {
  if (not MT2_had_isLoaded) {
    if (MT2_had_branch != 0) {
      MT2_had_branch->GetEntry(index);
    } else {
      printf("branch MT2_had_branch does not exist!\n");
      exit(1);
    }
    MT2_had_isLoaded = true;
  }
  return MT2_had_;
}

const float &StopCMS3::MT2_had_genmet() {
  if (not MT2_had_genmet_isLoaded) {
    if (MT2_had_genmet_branch != 0) {
      MT2_had_genmet_branch->GetEntry(index);
    } else {
      printf("branch MT2_had_genmet_branch does not exist!\n");
      exit(1);
    }
    MT2_had_genmet_isLoaded = true;
  }
  return MT2_had_genmet_;
}

const float &StopCMS3::MT2_had_jup() {
  if (not MT2_had_jup_isLoaded) {
    if (MT2_had_jup_branch != 0) {
      MT2_had_jup_branch->GetEntry(index);
    } else {
      printf("branch MT2_had_jup_branch does not exist!\n");
      exit(1);
    }
    MT2_had_jup_isLoaded = true;
  }
  return MT2_had_jup_;
}

const float &StopCMS3::MT2_had_jdown() {
  if (not MT2_had_jdown_isLoaded) {
    if (MT2_had_jdown_branch != 0) {
      MT2_had_jdown_branch->GetEntry(index);
    } else {
      printf("branch MT2_had_jdown_branch does not exist!\n");
      exit(1);
    }
    MT2_had_jdown_isLoaded = true;
  }
  return MT2_had_jdown_;
}

const float &StopCMS3::mht_pt() {
...
const float &StopCMS3::mht_pt_jup() {
  if (not mht_pt_jup_isLoaded) {
    if (mht_pt_jup_branch != 0) {
      mht_pt_jup_branch->GetEntry(index);
    } else {
      printf("branch mht_pt_jup_branch does not exist!\n");
      exit(1);
    }
    mht_pt_jup_isLoaded = true;
  }
  return mht_pt_jup_;
}

const float &StopCMS3::mht_pt_jdown() {
  if (not mht_pt_jdown_isLoaded) {
    if (mht_pt_jdown_branch != 0) {
      mht_pt_jdown_branch->GetEntry(index);
    } else {
      printf("branch mht_pt_jdown_branch does not exist!\n");
      exit(1);
    }
    mht_pt_jdown_isLoaded = true;
  }
  return mht_pt_jdown_;
}

const float &StopCMS3::mht_phi() {
...
const float &StopCMS3::mht_phi_jup() {
  if (not mht_phi_jup_isLoaded) {
    if (mht_phi_jup_branch != 0) {
      mht_phi_jup_branch->GetEntry(index);
    } else {
      printf("branch mht_phi_jup_branch does not exist!\n");
      exit(1);
    }
    mht_phi_jup_isLoaded = true;
  }
  return mht_phi_jup_;
}

const float &StopCMS3::mht_phi_jdown() {
  if (not mht_phi_jdown_isLoaded) {
    if (mht_phi_jdown_branch != 0) {
      mht_phi_jdown_branch->GetEntry(index);
    } else {
      printf("branch mht_phi_jdown_branch does not exist!\n");
      exit(1);
    }
    mht_phi_jdown_isLoaded = true;
  }
  return mht_phi_jdown_;
}

const float &StopCMS3::pfmet_egclean() {
...
const float &StopCMS3::pfmet_egclean_phi() {
...
const float &StopCMS3::pfmet_muegclean() {
...
const float &StopCMS3::pfmet_muegclean_phi() {
...
const float &StopCMS3::pfmet_muegcleanfix() {
...
const float &StopCMS3::pfmet_muegcleanfix_phi() {
...
const unsigned int &run() { return .run(); }
const unsigned int &ls() { return .ls(); }
const unsigned int &evt() { return .evt(); }
const int &nvtxs() { return .nvtxs(); }
const int &year() { return .year(); }
const int &sampver() { return .sampver(); }
const int &pu_nvtxs() { return .pu_nvtxs(); }
const float &pfmet() { return .pfmet(); }
const float &pfmet_phi() { return .pfmet_phi(); }
const float &pfmet_jup() { return .pfmet_jup(); }
const float &pfmet_phi_jup() { return .pfmet_phi_jup(); }
const float &pfmet_jdown() { return .pfmet_jdown(); }
const float &pfmet_phi_jdown() { return .pfmet_phi_jdown(); }
const float &pfmet_rl() { return .pfmet_rl(); }
const float &pfmet_phi_rl() { return .pfmet_phi_rl(); }
const float &pfmet_rl_jup() { return .pfmet_rl_jup(); }
const float &pfmet_phi_rl_jup() { return .pfmet_phi_rl_jup(); }
const float &pfmet_rl_jdown() { return .pfmet_rl_jdown(); }
const float &pfmet_phi_rl_jdown() { return .pfmet_phi_rl_jdown(); }
const float &pfmet_uncorr() { return .pfmet_uncorr(); }
const float &pfmet_uncorr_phi() { return .pfmet_uncorr_phi(); }
const float &pfmet_original() { return .pfmet_original(); }
const float &pfmet_original_phi() { return .pfmet_original_phi(); }
const float &scale1fb() { return .scale1fb(); }
const float &xsec() { return .xsec(); }
const float &xsec_uncert() { return .xsec_uncert(); }
const float &kfactor() { return .kfactor(); }
const float &pu_ntrue() { return .pu_ntrue(); }
const int &ngoodleps() { return .ngoodleps(); }
const int &nlooseleps() { return .nlooseleps(); }
const int &nvetoleps() { return .nvetoleps(); }
const bool &is_data() { return .is_data(); }
const string &dataset() { return .dataset(); }
const string &filename() { return .filename(); }
const string &cms3tag() { return .cms3tag(); }
const unsigned int &nEvents() { return .nEvents(); }
const unsigned int &nEvents_goodvtx() { return .nEvents_goodvtx(); }
const unsigned int &nEvents_MET30() { return .nEvents_MET30(); }
const unsigned int &nEvents_1goodlep() { return .nEvents_1goodlep(); }
const unsigned int &nEvents_2goodjets() { return .nEvents_2goodjets(); }
const int &is0lep() { return .is0lep(); }
const int &is1lep() { return .is1lep(); }
const int &is2lep() { return .is2lep(); }
const int &isZtoNuNu() { return .isZtoNuNu(); }
const int &is1lepFromW() { return .is1lepFromW(); }
const int &is1lepFromTop() { return .is1lepFromTop(); }
const float &MT2W() { return .MT2W(); }
const float &MT2W_rl() { return .MT2W_rl(); }
const float &MT2_ll() { return .MT2_ll(); }
const float &mindphi_met_j1_j2() { return .mindphi_met_j1_j2(); }
const float &mindphi_met_j1_j2_rl() { return .mindphi_met_j1_j2_rl(); }
const float &mt_met_lep() { return .mt_met_lep(); }
const float &mt_met_lep_rl() { return .mt_met_lep_rl(); }
const float &MT2W_jup() { return .MT2W_jup(); }
const float &MT2W_rl_jup() { return .MT2W_rl_jup(); }
const float &MT2_ll_jup() { return .MT2_ll_jup(); }
const float &mindphi_met_j1_j2_jup() { return .mindphi_met_j1_j2_jup(); }
const float &mindphi_met_j1_j2_rl_jup() { return .mindphi_met_j1_j2_rl_jup(); }
const float &mt_met_lep_jup() { return .mt_met_lep_jup(); }
const float &mt_met_lep_rl_jup() { return .mt_met_lep_rl_jup(); }
const float &MT2W_jdown() { return .MT2W_jdown(); }
const float &MT2W_rl_jdown() { return .MT2W_rl_jdown(); }
const float &MT2_ll_jdown() { return .MT2_ll_jdown(); }
const float &mindphi_met_j1_j2_jdown() { return .mindphi_met_j1_j2_jdown(); }
const float &mindphi_met_j1_j2_rl_jdown() { return .mindphi_met_j1_j2_rl_jdown(); }
const float &mt_met_lep_jdown() { return .mt_met_lep_jdown(); }
const float &mt_met_lep_rl_jdown() { return .mt_met_lep_rl_jdown(); }
const float &hadronic_top_chi2() { return .hadronic_top_chi2(); }
const float &ak4pfjets_rho() { return .ak4pfjets_rho(); }
const float &pdf_up_weight() { return .pdf_up_weight(); }
const float &pdf_down_weight() { return .pdf_down_weight(); }
const vector<string> &genweightsID() { return .genweightsID(); }
const int &ngenweights() { return .ngenweights(); }
const vector<float> &genweights() { return .genweights(); }
const float &weight_Q2_up() { return .weight_Q2_up(); }
const float &weight_Q2_down() { return .weight_Q2_down(); }
const float &weight_muF_up() { return .weight_muF_up(); }
const float &weight_muF_down() { return .weight_muF_down(); }
const float &weight_muR_up() { return .weight_muR_up(); }
const float &weight_muR_down() { return .weight_muR_down(); }
const float &weight_alphas_up() { return .weight_alphas_up(); }
const float &weight_alphas_down() { return .weight_alphas_down(); }
const float &weight_btagsf() { return .weight_btagsf(); }
const float &weight_btagsf_heavy_UP() { return .weight_btagsf_heavy_UP(); }
const float &weight_btagsf_light_UP() { return .weight_btagsf_light_UP(); }
const float &weight_btagsf_heavy_DN() { return .weight_btagsf_heavy_DN(); }
const float &weight_btagsf_light_DN() { return .weight_btagsf_light_DN(); }
const float &weight_btagsf_fastsim_UP() { return .weight_btagsf_fastsim_UP(); }
const float &weight_btagsf_fastsim_DN() { return .weight_btagsf_fastsim_DN(); }
const float &weight_analysisbtagsf() { return .weight_analysisbtagsf(); }
const float &weight_analysisbtagsf_heavy_UP() { return .weight_analysisbtagsf_heavy_UP(); }
const float &weight_analysisbtagsf_light_UP() { return .weight_analysisbtagsf_light_UP(); }
const float &weight_analysisbtagsf_heavy_DN() { return .weight_analysisbtagsf_heavy_DN(); }
const float &weight_analysisbtagsf_light_DN() { return .weight_analysisbtagsf_light_DN(); }
const float &weight_analysisbtagsf_fastsim_UP() { return .weight_analysisbtagsf_fastsim_UP(); }
const float &weight_analysisbtagsf_fastsim_DN() { return .weight_analysisbtagsf_fastsim_DN(); }
const float &weight_tightbtagsf() { return .weight_tightbtagsf(); }
const float &weight_tightbtagsf_heavy_UP() { return .weight_tightbtagsf_heavy_UP(); }
const float &weight_tightbtagsf_light_UP() { return .weight_tightbtagsf_light_UP(); }
const float &weight_tightbtagsf_heavy_DN() { return .weight_tightbtagsf_heavy_DN(); }
const float &weight_tightbtagsf_light_DN() { return .weight_tightbtagsf_light_DN(); }
const float &weight_tightbtagsf_fastsim_UP() { return .weight_tightbtagsf_fastsim_UP(); }
const float &weight_tightbtagsf_fastsim_DN() { return .weight_tightbtagsf_fastsim_DN(); }
const float &weight_loosebtagsf() { return .weight_loosebtagsf(); }
const float &weight_loosebtagsf_heavy_UP() { return .weight_loosebtagsf_heavy_UP(); }
const float &weight_loosebtagsf_light_UP() { return .weight_loosebtagsf_light_UP(); }
const float &weight_loosebtagsf_heavy_DN() { return .weight_loosebtagsf_heavy_DN(); }
const float &weight_loosebtagsf_light_DN() { return .weight_loosebtagsf_light_DN(); }
const float &weight_loosebtagsf_fastsim_UP() { return .weight_loosebtagsf_fastsim_UP(); }
const float &weight_loosebtagsf_fastsim_DN() { return .weight_loosebtagsf_fastsim_DN(); }
const float &weight_lepSF() { return .weight_lepSF(); }
const float &weight_lepSF_up() { return .weight_lepSF_up(); }
const float &weight_lepSF_down() { return .weight_lepSF_down(); }
const float &weight_vetoLepSF() { return .weight_vetoLepSF(); }
const float &weight_vetoLepSF_up() { return .weight_vetoLepSF_up(); }
const float &weight_vetoLepSF_down() { return .weight_vetoLepSF_down(); }
const float &weight_lepSF_fastSim() { return .weight_lepSF_fastSim(); }
const float &weight_lepSF_fastSim_up() { return .weight_lepSF_fastSim_up(); }
const float &weight_lepSF_fastSim_down() { return .weight_lepSF_fastSim_down(); }
const float &weight_ISR() { return .weight_ISR(); }
const float &weight_ISRup() { return .weight_ISRup(); }
const float &weight_ISRdown() { return .weight_ISRdown(); }
const float &weight_PU() { return .weight_PU(); }
const float &weight_PUup() { return .weight_PUup(); }
const float &weight_PUdown() { return .weight_PUdown(); }
const float &weight_ISRnjets() { return .weight_ISRnjets(); }
const float &weight_ISRnjets_UP() { return .weight_ISRnjets_UP(); }
const float &weight_ISRnjets_DN() { return .weight_ISRnjets_DN(); }
const float &weight_L1prefire() { return .weight_L1prefire(); }
const float &weight_L1prefire_UP() { return .weight_L1prefire_UP(); }
const float &weight_L1prefire_DN() { return .weight_L1prefire_DN(); }
const int &NISRjets() { return .NISRjets(); }
const int &NnonISRjets() { return .NnonISRjets(); }
const vector<string> &sparms_names() { return .sparms_names(); }
const vector<float> &sparms_values() { return .sparms_values(); }
const int &sparms_subProcessId() { return .sparms_subProcessId(); }
const float &mass_lsp() { return .mass_lsp(); }
const float &mass_chargino() { return .mass_chargino(); }
const float &mass_stop() { return .mass_stop(); }
const float &mass_gluino() { return .mass_gluino(); }
const float &genmet() { return .genmet(); }
const float &genmet_phi() { return .genmet_phi(); }
const float &nupt() { return .nupt(); }
const float &genht() { return .genht(); }
const bool &PassTrackVeto() { return .PassTrackVeto(); }
const bool &PassTauVeto() { return .PassTauVeto(); }
const float &topness() { return .topness(); }
const float &topnessMod() { return .topnessMod(); }
const float &topnessMod_rl() { return .topnessMod_rl(); }
const float &topnessMod_jup() { return .topnessMod_jup(); }
const float &topnessMod_rl_jup() { return .topnessMod_rl_jup(); }
const float &topnessMod_jdown() { return .topnessMod_jdown(); }
const float &topnessMod_rl_jdown() { return .topnessMod_rl_jdown(); }
const float &Mlb_closestb() { return .Mlb_closestb(); }
const float &Mlb_lead_bdiscr() { return .Mlb_lead_bdiscr(); }
const float &Mlb_closestb_jup() { return .Mlb_closestb_jup(); }
const float &Mlb_lead_bdiscr_jup() { return .Mlb_lead_bdiscr_jup(); }
const float &Mlb_closestb_jdown() { return .Mlb_closestb_jdown(); }
const float &Mlb_lead_bdiscr_jdown() { return .Mlb_lead_bdiscr_jdown(); }
const int &HLT_SingleEl() { return .HLT_SingleEl(); }
const int &HLT_SingleMu() { return .HLT_SingleMu(); }
const int &HLT_MET() { return .HLT_MET(); }
const int &HLT_MET_MHT() { return .HLT_MET_MHT(); }
const int &HLT_MET100_MHT100() { return .HLT_MET100_MHT100(); }
const int &HLT_MET110_MHT110() { return .HLT_MET110_MHT110(); }
const int &HLT_MET120_MHT120() { return .HLT_MET120_MHT120(); }
const int &HLT_MET130_MHT130() { return .HLT_MET130_MHT130(); }
const int &HLT_PFHT_unprescaled() { return .HLT_PFHT_unprescaled(); }
const int &HLT_PFHT_prescaled() { return .HLT_PFHT_prescaled(); }
const int &HLT_AK8Jet_unprescaled() { return .HLT_AK8Jet_unprescaled(); }
const int &HLT_AK8Jet_prescaled() { return .HLT_AK8Jet_prescaled(); }
const int &HLT_CaloJet500_NoJetID() { return .HLT_CaloJet500_NoJetID(); }
const int &HLT_DiEl() { return .HLT_DiEl(); }
const int &HLT_DiMu() { return .HLT_DiMu(); }
const int &HLT_MuE() { return .HLT_MuE(); }
const int &nPhotons() { return .nPhotons(); }
const int &ph_ngoodjets() { return .ph_ngoodjets(); }
const int &ph_ngoodbtags() { return .ph_ngoodbtags(); }
const float &hardgenpt() { return .hardgenpt(); }
const float &calomet() { return .calomet(); }
const float &calomet_phi() { return .calomet_phi(); }
const int &lep1_pdgid() { return .lep1_pdgid(); }
const int &lep1_production_type() { return .lep1_production_type(); }
const float &lep1_MiniIso() { return .lep1_MiniIso(); }
const float &lep1_relIso() { return .lep1_relIso(); }
const bool &lep1_passLooseID() { return .lep1_passLooseID(); }
const bool &lep1_passMediumID() { return .lep1_passMediumID(); }
const bool &lep1_passTightID() { return .lep1_passTightID(); }
const bool &lep1_passVeto() { return .lep1_passVeto(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &lep1_p4() { return .lep1_p4(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &lep1_mcp4() { return .lep1_mcp4(); }
const int &lep1_mc_motherid() { return .lep1_mc_motherid(); }
const float &lep1_etaSC() { return .lep1_etaSC(); }
const float &lep1_dphiMET() { return .lep1_dphiMET(); }
const float &lep1_dphiMET_jup() { return .lep1_dphiMET_jup(); }
const float &lep1_dphiMET_jdown() { return .lep1_dphiMET_jdown(); }
const float &lep1_dphiMET_rl() { return .lep1_dphiMET_rl(); }
const float &lep1_dphiMET_rl_jup() { return .lep1_dphiMET_rl_jup(); }
const float &lep1_dphiMET_rl_jdown() { return .lep1_dphiMET_rl_jdown(); }
const float &lep1_dphiMET_resup() { return .lep1_dphiMET_resup(); }
const float &lep1_dphiMET_resdown() { return .lep1_dphiMET_resdown(); }
const float &lep1_dphiMET_rl_resup() { return .lep1_dphiMET_rl_resup(); }
const float &lep1_dphiMET_rl_resdown() { return .lep1_dphiMET_rl_resdown(); }
const float &lep1_dphiGenMET() { return .lep1_dphiGenMET(); }
const float &lep1_dphiGenMET_rl() { return .lep1_dphiGenMET_rl(); }
const int &lep2_pdgid() { return .lep2_pdgid(); }
const int &lep2_production_type() { return .lep2_production_type(); }
const float &lep2_MiniIso() { return .lep2_MiniIso(); }
const float &lep2_relIso() { return .lep2_relIso(); }
const bool &lep2_passLooseID() { return .lep2_passLooseID(); }
const bool &lep2_passMediumID() { return .lep2_passMediumID(); }
const bool &lep2_passTightID() { return .lep2_passTightID(); }
const bool &lep2_passVeto() { return .lep2_passVeto(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &lep2_p4() { return .lep2_p4(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &lep2_mcp4() { return .lep2_mcp4(); }
const int &lep2_mc_motherid() { return .lep2_mc_motherid(); }
const float &lep2_etaSC() { return .lep2_etaSC(); }
const float &lep2_dphiMET() { return .lep2_dphiMET(); }
const float &lep2_dphiMET_jup() { return .lep2_dphiMET_jup(); }
const float &lep2_dphiMET_jdown() { return .lep2_dphiMET_jdown(); }
const float &lep2_dphiMET_rl() { return .lep2_dphiMET_rl(); }
const float &lep2_dphiMET_rl_jup() { return .lep2_dphiMET_rl_jup(); }
const float &lep2_dphiMET_rl_jdown() { return .lep2_dphiMET_rl_jdown(); }
const float &lep2_dphiMET_resup() { return .lep2_dphiMET_resup(); }
const float &lep2_dphiMET_resdown() { return .lep2_dphiMET_resdown(); }
const float &lep2_dphiMET_rl_resup() { return .lep2_dphiMET_rl_resup(); }
const float &lep2_dphiMET_rl_resdown() { return .lep2_dphiMET_rl_resdown(); }
const float &lep2_dphiGenMET() { return .lep2_dphiGenMET(); }
const float &lep2_dphiGenMET_rl() { return .lep2_dphiGenMET_rl(); }
const vector<float> &ph_sigmaIEtaEta_fill5x5() { return .ph_sigmaIEtaEta_fill5x5(); }
const vector<float> &ph_hOverE() { return .ph_hOverE(); }
const vector<float> &ph_r9() { return .ph_r9(); }
const vector<float> &ph_chiso() { return .ph_chiso(); }
const vector<float> &ph_nhiso() { return .ph_nhiso(); }
const vector<float> &ph_phiso() { return .ph_phiso(); }
const vector<bool> &ph_passLooseID() { return .ph_passLooseID(); }
const vector<bool> &ph_passMediumID() { return .ph_passMediumID(); }
const vector<bool> &ph_passTightID() { return .ph_passTightID(); }
const vector<int> &ph_overlapJetId() { return .ph_overlapJetId(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &ph_p4() { return .ph_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &ph_mcp4() { return .ph_mcp4(); }
const vector<int> &ph_mcMatchId() { return .ph_mcMatchId(); }
const vector<float> &ph_genIso04() { return .ph_genIso04(); }
const vector<float> &ph_drMinParton() { return .ph_drMinParton(); }
const int &nskimjets() { return .nskimjets(); }
const int &nskimbtagmed() { return .nskimbtagmed(); }
const int &nskimbtagloose() { return .nskimbtagloose(); }
const int &nskimbtagtight() { return .nskimbtagtight(); }
const int &ngoodjets() { return .ngoodjets(); }
const int &ngoodbtags() { return .ngoodbtags(); }
const int &nloosebtags() { return .nloosebtags(); }
const int &ntightbtags() { return .ntightbtags(); }
const int &nanalysisbtags() { return .nanalysisbtags(); }
const int &nfailjets() { return .nfailjets(); }
const float &ak4_HT() { return .ak4_HT(); }
const float &ak4_HTeta5() { return .ak4_HTeta5(); }
const float &ak4_MHT_pt() { return .ak4_MHT_pt(); }
const float &ak4_MHT_phi() { return .ak4_MHT_phi(); }
const float &ak4_htratiom() { return .ak4_htratiom(); }
const vector<float> &dphi_ak4pfjet_met() { return .dphi_ak4pfjet_met(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &ak4pfjets_p4() { return .ak4pfjets_p4(); }
const vector<bool> &ak4pfjets_passMEDbtag() { return .ak4pfjets_passMEDbtag(); }
const vector<float> &ak4pfjets_deepCSV() { return .ak4pfjets_deepCSV(); }
const vector<float> &ak4pfjets_CSV() { return .ak4pfjets_CSV(); }
const vector<float> &ak4pfjets_mva() { return .ak4pfjets_mva(); }
const vector<int> &ak4pfjets_parton_flavor() { return .ak4pfjets_parton_flavor(); }
const vector<int> &ak4pfjets_hadron_flavor() { return .ak4pfjets_hadron_flavor(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &ak4pfjets_leadMEDbjet_p4() { return .ak4pfjets_leadMEDbjet_p4(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &ak4pfjets_leadbtag_p4() { return .ak4pfjets_leadbtag_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &ak4genjets_p4() { return .ak4genjets_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &softtags_p4() { return .softtags_p4(); }
const int &nsoftbtags() { return .nsoftbtags(); }
const int &jup_nskimjets() { return .jup_nskimjets(); }
const int &jup_nskimbtagmed() { return .jup_nskimbtagmed(); }
const int &jup_nskimbtagloose() { return .jup_nskimbtagloose(); }
const int &jup_nskimbtagtight() { return .jup_nskimbtagtight(); }
const int &jup_ngoodjets() { return .jup_ngoodjets(); }
const int &jup_ngoodbtags() { return .jup_ngoodbtags(); }
const int &jup_nloosebtags() { return .jup_nloosebtags(); }
const int &jup_ntightbtags() { return .jup_ntightbtags(); }
const int &jup_nanalysisbtags() { return .jup_nanalysisbtags(); }
const int &jup_nfailjets() { return .jup_nfailjets(); }
const float &jup_ak4_HT() { return .jup_ak4_HT(); }
const float &jup_ak4_HTeta5() { return .jup_ak4_HTeta5(); }
const float &jup_ak4_MHT_pt() { return .jup_ak4_MHT_pt(); }
const float &jup_ak4_MHT_phi() { return .jup_ak4_MHT_phi(); }
const float &jup_ak4_htratiom() { return .jup_ak4_htratiom(); }
const vector<float> &jup_dphi_ak4pfjet_met() { return .jup_dphi_ak4pfjet_met(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jup_ak4pfjets_p4() { return .jup_ak4pfjets_p4(); }
const vector<bool> &jup_ak4pfjets_passMEDbtag() { return .jup_ak4pfjets_passMEDbtag(); }
const vector<float> &jup_ak4pfjets_deepCSV() { return .jup_ak4pfjets_deepCSV(); }
const vector<float> &jup_ak4pfjets_CSV() { return .jup_ak4pfjets_CSV(); }
const vector<float> &jup_ak4pfjets_mva() { return .jup_ak4pfjets_mva(); }
const vector<int> &jup_ak4pfjets_parton_flavor() { return .jup_ak4pfjets_parton_flavor(); }
const vector<int> &jup_ak4pfjets_hadron_flavor() { return .jup_ak4pfjets_hadron_flavor(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &jup_ak4pfjets_leadMEDbjet_p4() { return .jup_ak4pfjets_leadMEDbjet_p4(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &jup_ak4pfjets_leadbtag_p4() { return .jup_ak4pfjets_leadbtag_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jup_ak4genjets_p4() { return .jup_ak4genjets_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jup_softtags_p4() { return .jup_softtags_p4(); }
const int &jup_nsoftbtags() { return .jup_nsoftbtags(); }
const int &jdown_nskimjets() { return .jdown_nskimjets(); }
const int &jdown_nskimbtagmed() { return .jdown_nskimbtagmed(); }
const int &jdown_nskimbtagloose() { return .jdown_nskimbtagloose(); }
const int &jdown_nskimbtagtight() { return .jdown_nskimbtagtight(); }
const int &jdown_ngoodjets() { return .jdown_ngoodjets(); }
const int &jdown_ngoodbtags() { return .jdown_ngoodbtags(); }
const int &jdown_nloosebtags() { return .jdown_nloosebtags(); }
const int &jdown_ntightbtags() { return .jdown_ntightbtags(); }
const int &jdown_nanalysisbtags() { return .jdown_nanalysisbtags(); }
const int &jdown_nfailjets() { return .jdown_nfailjets(); }
const float &jdown_ak4_HT() { return .jdown_ak4_HT(); }
const float &jdown_ak4_HTeta5() { return .jdown_ak4_HTeta5(); }
const float &jdown_ak4_MHT_pt() { return .jdown_ak4_MHT_pt(); }
const float &jdown_ak4_MHT_phi() { return .jdown_ak4_MHT_phi(); }
const float &jdown_ak4_htratiom() { return .jdown_ak4_htratiom(); }
const vector<float> &jdown_dphi_ak4pfjet_met() { return .jdown_dphi_ak4pfjet_met(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jdown_ak4pfjets_p4() { return .jdown_ak4pfjets_p4(); }
const vector<bool> &jdown_ak4pfjets_passMEDbtag() { return .jdown_ak4pfjets_passMEDbtag(); }
const vector<float> &jdown_ak4pfjets_deepCSV() { return .jdown_ak4pfjets_deepCSV(); }
const vector<float> &jdown_ak4pfjets_CSV() { return .jdown_ak4pfjets_CSV(); }
const vector<float> &jdown_ak4pfjets_mva() { return .jdown_ak4pfjets_mva(); }
const vector<int> &jdown_ak4pfjets_parton_flavor() { return .jdown_ak4pfjets_parton_flavor(); }
const vector<int> &jdown_ak4pfjets_hadron_flavor() { return .jdown_ak4pfjets_hadron_flavor(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &jdown_ak4pfjets_leadMEDbjet_p4() { return .jdown_ak4pfjets_leadMEDbjet_p4(); }
const ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > &jdown_ak4pfjets_leadbtag_p4() { return .jdown_ak4pfjets_leadbtag_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jdown_ak4genjets_p4() { return .jdown_ak4genjets_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jdown_softtags_p4() { return .jdown_softtags_p4(); }
const int &jdown_nsoftbtags() { return .jdown_nsoftbtags(); }
const vector<bool> &genleps_isfromt() { return .genleps_isfromt(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genleps_p4() { return .genleps_p4(); }
const vector<int> &genleps_id() { return .genleps_id(); }
const vector<int> &genleps__genpsidx() { return .genleps__genpsidx(); }
const vector<int> &genleps_status() { return .genleps_status(); }
const vector<bool> &genleps_fromHardProcessDecayed() { return .genleps_fromHardProcessDecayed(); }
const vector<bool> &genleps_fromHardProcessFinalState() { return .genleps_fromHardProcessFinalState(); }
const vector<bool> &genleps_isHardProcess() { return .genleps_isHardProcess(); }
const vector<bool> &genleps_isLastCopy() { return .genleps_isLastCopy(); }
const vector<int> &genleps_gentaudecay() { return .genleps_gentaudecay(); }
const int &gen_nfromtleps_() { return .gen_nfromtleps_(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genleps_motherp4() { return .genleps_motherp4(); }
const vector<int> &genleps_motherid() { return .genleps_motherid(); }
const vector<int> &genleps_motheridx() { return .genleps_motheridx(); }
const vector<int> &genleps_motherstatus() { return .genleps_motherstatus(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genleps_gmotherp4() { return .genleps_gmotherp4(); }
const vector<int> &genleps_gmotherid() { return .genleps_gmotherid(); }
const vector<int> &genleps_gmotheridx() { return .genleps_gmotheridx(); }
const vector<int> &genleps_gmotherstatus() { return .genleps_gmotherstatus(); }
const vector<bool> &gennus_isfromt() { return .gennus_isfromt(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &gennus_p4() { return .gennus_p4(); }
const vector<int> &gennus_id() { return .gennus_id(); }
const vector<int> &gennus__genpsidx() { return .gennus__genpsidx(); }
const vector<int> &gennus_status() { return .gennus_status(); }
const vector<bool> &gennus_fromHardProcessDecayed() { return .gennus_fromHardProcessDecayed(); }
const vector<bool> &gennus_fromHardProcessFinalState() { return .gennus_fromHardProcessFinalState(); }
const vector<bool> &gennus_isHardProcess() { return .gennus_isHardProcess(); }
const vector<bool> &gennus_isLastCopy() { return .gennus_isLastCopy(); }
const vector<int> &gennus_gentaudecay() { return .gennus_gentaudecay(); }
const int &gen_nfromtnus_() { return .gen_nfromtnus_(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &gennus_motherp4() { return .gennus_motherp4(); }
const vector<int> &gennus_motherid() { return .gennus_motherid(); }
const vector<int> &gennus_motheridx() { return .gennus_motheridx(); }
const vector<int> &gennus_motherstatus() { return .gennus_motherstatus(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &gennus_gmotherp4() { return .gennus_gmotherp4(); }
const vector<int> &gennus_gmotherid() { return .gennus_gmotherid(); }
const vector<int> &gennus_gmotheridx() { return .gennus_gmotheridx(); }
const vector<int> &gennus_gmotherstatus() { return .gennus_gmotherstatus(); }
const vector<bool> &genqs_isfromt() { return .genqs_isfromt(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genqs_p4() { return .genqs_p4(); }
const vector<int> &genqs_id() { return .genqs_id(); }
const vector<int> &genqs__genpsidx() { return .genqs__genpsidx(); }
const vector<int> &genqs_status() { return .genqs_status(); }
const vector<bool> &genqs_fromHardProcessDecayed() { return .genqs_fromHardProcessDecayed(); }
const vector<bool> &genqs_fromHardProcessFinalState() { return .genqs_fromHardProcessFinalState(); }
const vector<bool> &genqs_isHardProcess() { return .genqs_isHardProcess(); }
const vector<bool> &genqs_isLastCopy() { return .genqs_isLastCopy(); }
const vector<int> &genqs_gentaudecay() { return .genqs_gentaudecay(); }
const int &gen_nfromtqs_() { return .gen_nfromtqs_(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genqs_motherp4() { return .genqs_motherp4(); }
const vector<int> &genqs_motherid() { return .genqs_motherid(); }
const vector<int> &genqs_motheridx() { return .genqs_motheridx(); }
const vector<int> &genqs_motherstatus() { return .genqs_motherstatus(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genqs_gmotherp4() { return .genqs_gmotherp4(); }
const vector<int> &genqs_gmotherid() { return .genqs_gmotherid(); }
const vector<int> &genqs_gmotheridx() { return .genqs_gmotheridx(); }
const vector<int> &genqs_gmotherstatus() { return .genqs_gmotherstatus(); }
const vector<bool> &genbosons_isfromt() { return .genbosons_isfromt(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genbosons_p4() { return .genbosons_p4(); }
const vector<int> &genbosons_id() { return .genbosons_id(); }
const vector<int> &genbosons__genpsidx() { return .genbosons__genpsidx(); }
const vector<int> &genbosons_status() { return .genbosons_status(); }
const vector<bool> &genbosons_fromHardProcessDecayed() { return .genbosons_fromHardProcessDecayed(); }
const vector<bool> &genbosons_fromHardProcessFinalState() { return .genbosons_fromHardProcessFinalState(); }
const vector<bool> &genbosons_isHardProcess() { return .genbosons_isHardProcess(); }
const vector<bool> &genbosons_isLastCopy() { return .genbosons_isLastCopy(); }
const vector<int> &genbosons_gentaudecay() { return .genbosons_gentaudecay(); }
const int &gen_nfromtbosons_() { return .gen_nfromtbosons_(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genbosons_motherp4() { return .genbosons_motherp4(); }
const vector<int> &genbosons_motherid() { return .genbosons_motherid(); }
const vector<int> &genbosons_motheridx() { return .genbosons_motheridx(); }
const vector<int> &genbosons_motherstatus() { return .genbosons_motherstatus(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &genbosons_gmotherp4() { return .genbosons_gmotherp4(); }
const vector<int> &genbosons_gmotherid() { return .genbosons_gmotherid(); }
const vector<int> &genbosons_gmotheridx() { return .genbosons_gmotheridx(); }
const vector<int> &genbosons_gmotherstatus() { return .genbosons_gmotherstatus(); }
const vector<bool> &gensusy_isfromt() { return .gensusy_isfromt(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &gensusy_p4() { return .gensusy_p4(); }
const vector<int> &gensusy_id() { return .gensusy_id(); }
const vector<int> &gensusy__genpsidx() { return .gensusy__genpsidx(); }
const vector<int> &gensusy_status() { return .gensusy_status(); }
const vector<bool> &gensusy_fromHardProcessDecayed() { return .gensusy_fromHardProcessDecayed(); }
const vector<bool> &gensusy_fromHardProcessFinalState() { return .gensusy_fromHardProcessFinalState(); }
const vector<bool> &gensusy_isHardProcess() { return .gensusy_isHardProcess(); }
const vector<bool> &gensusy_isLastCopy() { return .gensusy_isLastCopy(); }
const vector<int> &gensusy_gentaudecay() { return .gensusy_gentaudecay(); }
const int &gen_nfromtsusy_() { return .gen_nfromtsusy_(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &gensusy_motherp4() { return .gensusy_motherp4(); }
const vector<int> &gensusy_motherid() { return .gensusy_motherid(); }
const vector<int> &gensusy_motheridx() { return .gensusy_motheridx(); }
const vector<int> &gensusy_motherstatus() { return .gensusy_motherstatus(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &gensusy_gmotherp4() { return .gensusy_gmotherp4(); }
const vector<int> &gensusy_gmotherid() { return .gensusy_gmotherid(); }
const vector<int> &gensusy_gmotheridx() { return .gensusy_gmotheridx(); }
const vector<int> &gensusy_gmotherstatus() { return .gensusy_gmotherstatus(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &ak8pfjets_p4() { return .ak8pfjets_p4(); }
const vector<float> &ak8pfjets_tau1() { return .ak8pfjets_tau1(); }
const vector<float> &ak8pfjets_tau2() { return .ak8pfjets_tau2(); }
const vector<float> &ak8pfjets_tau3() { return .ak8pfjets_tau3(); }
const vector<int> &ak8pfjets_parton_flavor() { return .ak8pfjets_parton_flavor(); }
const int &nGoodAK8PFJets() { return .nGoodAK8PFJets(); }
const vector<float> &ak8pfjets_deepdisc_qcd() { return .ak8pfjets_deepdisc_qcd(); }
const vector<float> &ak8pfjets_deepdisc_top() { return .ak8pfjets_deepdisc_top(); }
const vector<float> &ak8pfjets_deepdisc_w() { return .ak8pfjets_deepdisc_w(); }
const vector<float> &ak8pfjets_deepdisc_z() { return .ak8pfjets_deepdisc_z(); }
const vector<float> &ak8pfjets_deepdisc_zbb() { return .ak8pfjets_deepdisc_zbb(); }
const vector<float> &ak8pfjets_deepdisc_hbb() { return .ak8pfjets_deepdisc_hbb(); }
const vector<float> &ak8pfjets_deepdisc_h4q() { return .ak8pfjets_deepdisc_h4q(); }
const float &lead_ak8deepdisc_top() { return .lead_ak8deepdisc_top(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jup_ak8pfjets_p4() { return .jup_ak8pfjets_p4(); }
const vector<float> &jup_ak8pfjets_tau1() { return .jup_ak8pfjets_tau1(); }
const vector<float> &jup_ak8pfjets_tau2() { return .jup_ak8pfjets_tau2(); }
const vector<float> &jup_ak8pfjets_tau3() { return .jup_ak8pfjets_tau3(); }
const vector<int> &jup_ak8pfjets_parton_flavor() { return .jup_ak8pfjets_parton_flavor(); }
const int &jup_nGoodAK8PFJets() { return .jup_nGoodAK8PFJets(); }
const vector<float> &jup_ak8pfjets_deepdisc_qcd() { return .jup_ak8pfjets_deepdisc_qcd(); }
const vector<float> &jup_ak8pfjets_deepdisc_top() { return .jup_ak8pfjets_deepdisc_top(); }
const vector<float> &jup_ak8pfjets_deepdisc_w() { return .jup_ak8pfjets_deepdisc_w(); }
const vector<float> &jup_ak8pfjets_deepdisc_z() { return .jup_ak8pfjets_deepdisc_z(); }
const vector<float> &jup_ak8pfjets_deepdisc_zbb() { return .jup_ak8pfjets_deepdisc_zbb(); }
const vector<float> &jup_ak8pfjets_deepdisc_hbb() { return .jup_ak8pfjets_deepdisc_hbb(); }
const vector<float> &jup_ak8pfjets_deepdisc_h4q() { return .jup_ak8pfjets_deepdisc_h4q(); }
const float &jup_lead_ak8deepdisc_top() { return .jup_lead_ak8deepdisc_top(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jdown_ak8pfjets_p4() { return .jdown_ak8pfjets_p4(); }
const vector<float> &jdown_ak8pfjets_tau1() { return .jdown_ak8pfjets_tau1(); }
const vector<float> &jdown_ak8pfjets_tau2() { return .jdown_ak8pfjets_tau2(); }
const vector<float> &jdown_ak8pfjets_tau3() { return .jdown_ak8pfjets_tau3(); }
const vector<int> &jdown_ak8pfjets_parton_flavor() { return .jdown_ak8pfjets_parton_flavor(); }
const int &jdown_nGoodAK8PFJets() { return .jdown_nGoodAK8PFJets(); }
const vector<float> &jdown_ak8pfjets_deepdisc_qcd() { return .jdown_ak8pfjets_deepdisc_qcd(); }
const vector<float> &jdown_ak8pfjets_deepdisc_top() { return .jdown_ak8pfjets_deepdisc_top(); }
const vector<float> &jdown_ak8pfjets_deepdisc_w() { return .jdown_ak8pfjets_deepdisc_w(); }
const vector<float> &jdown_ak8pfjets_deepdisc_z() { return .jdown_ak8pfjets_deepdisc_z(); }
const vector<float> &jdown_ak8pfjets_deepdisc_zbb() { return .jdown_ak8pfjets_deepdisc_zbb(); }
const vector<float> &jdown_ak8pfjets_deepdisc_hbb() { return .jdown_ak8pfjets_deepdisc_hbb(); }
const vector<float> &jdown_ak8pfjets_deepdisc_h4q() { return .jdown_ak8pfjets_deepdisc_h4q(); }
const float &jdown_lead_ak8deepdisc_top() { return .jdown_lead_ak8deepdisc_top(); }
const vector<float> &ak4pfjets_cvsl() { return .ak4pfjets_cvsl(); }
const vector<float> &ak4pfjets_ptD() { return .ak4pfjets_ptD(); }
const vector<float> &ak4pfjets_axis1() { return .ak4pfjets_axis1(); }
const vector<float> &ak4pfjets_axis2() { return .ak4pfjets_axis2(); }
const vector<int> &ak4pfjets_mult() { return .ak4pfjets_mult(); }
const vector<float> &ak4pfjets_deepCSVb() { return .ak4pfjets_deepCSVb(); }
const vector<float> &ak4pfjets_deepCSVbb() { return .ak4pfjets_deepCSVbb(); }
const vector<float> &ak4pfjets_deepCSVc() { return .ak4pfjets_deepCSVc(); }
const vector<float> &ak4pfjets_deepCSVl() { return .ak4pfjets_deepCSVl(); }
const vector<vector<int> > &topcands_ak4idx() { return .topcands_ak4idx(); }
const vector<float> &topcands_disc() { return .topcands_disc(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &topcands_p4() { return .topcands_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &topcands_Wp4() { return .topcands_Wp4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &tftops_p4() { return .tftops_p4(); }
const vector<float> &tftops_disc() { return .tftops_disc(); }
const vector<vector<float> > &tftops_subjet_pt() { return .tftops_subjet_pt(); }
const vector<vector<float> > &tftops_subjet_eta() { return .tftops_subjet_eta(); }
const vector<vector<float> > &tftops_subjet_phi() { return .tftops_subjet_phi(); }
const vector<float> &jup_ak4pfjets_cvsl() { return .jup_ak4pfjets_cvsl(); }
const vector<float> &jup_ak4pfjets_ptD() { return .jup_ak4pfjets_ptD(); }
const vector<float> &jup_ak4pfjets_axis1() { return .jup_ak4pfjets_axis1(); }
const vector<float> &jup_ak4pfjets_axis2() { return .jup_ak4pfjets_axis2(); }
const vector<int> &jup_ak4pfjets_mult() { return .jup_ak4pfjets_mult(); }
const vector<float> &jup_ak4pfjets_deepCSVb() { return .jup_ak4pfjets_deepCSVb(); }
const vector<float> &jup_ak4pfjets_deepCSVbb() { return .jup_ak4pfjets_deepCSVbb(); }
const vector<float> &jup_ak4pfjets_deepCSVc() { return .jup_ak4pfjets_deepCSVc(); }
const vector<float> &jup_ak4pfjets_deepCSVl() { return .jup_ak4pfjets_deepCSVl(); }
const vector<vector<int> > &jup_topcands_ak4idx() { return .jup_topcands_ak4idx(); }
const vector<float> &jup_topcands_disc() { return .jup_topcands_disc(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jup_topcands_p4() { return .jup_topcands_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jup_topcands_Wp4() { return .jup_topcands_Wp4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jup_tftops_p4() { return .jup_tftops_p4(); }
const vector<float> &jup_tftops_disc() { return .jup_tftops_disc(); }
const vector<vector<float> > &jup_tftops_subjet_pt() { return .jup_tftops_subjet_pt(); }
const vector<vector<float> > &jup_tftops_subjet_eta() { return .jup_tftops_subjet_eta(); }
const vector<vector<float> > &jup_tftops_subjet_phi() { return .jup_tftops_subjet_phi(); }
const vector<float> &jdown_ak4pfjets_cvsl() { return .jdown_ak4pfjets_cvsl(); }
const vector<float> &jdown_ak4pfjets_ptD() { return .jdown_ak4pfjets_ptD(); }
const vector<float> &jdown_ak4pfjets_axis1() { return .jdown_ak4pfjets_axis1(); }
const vector<float> &jdown_ak4pfjets_axis2() { return .jdown_ak4pfjets_axis2(); }
const vector<int> &jdown_ak4pfjets_mult() { return .jdown_ak4pfjets_mult(); }
const vector<float> &jdown_ak4pfjets_deepCSVb() { return .jdown_ak4pfjets_deepCSVb(); }
const vector<float> &jdown_ak4pfjets_deepCSVbb() { return .jdown_ak4pfjets_deepCSVbb(); }
const vector<float> &jdown_ak4pfjets_deepCSVc() { return .jdown_ak4pfjets_deepCSVc(); }
const vector<float> &jdown_ak4pfjets_deepCSVl() { return .jdown_ak4pfjets_deepCSVl(); }
const vector<vector<int> > &jdown_topcands_ak4idx() { return .jdown_topcands_ak4idx(); }
const vector<float> &jdown_topcands_disc() { return .jdown_topcands_disc(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jdown_topcands_p4() { return .jdown_topcands_p4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jdown_topcands_Wp4() { return .jdown_topcands_Wp4(); }
const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jdown_tftops_p4() { return .jdown_tftops_p4(); }
const vector<float> &jdown_tftops_disc() { return .jdown_tftops_disc(); }
const vector<vector<float> > &jdown_tftops_subjet_pt() { return .jdown_tftops_subjet_pt(); }
const vector<vector<float> > &jdown_tftops_subjet_eta() { return .jdown_tftops_subjet_eta(); }
const vector<vector<float> > &jdown_tftops_subjet_phi() { return .jdown_tftops_subjet_phi(); }
const float &pfmet_uncorr_jup() { return .pfmet_uncorr_jup(); }
const float &pfmet_uncorr_phi_jup() { return .pfmet_uncorr_phi_jup(); }
const float &pfmet_uncorr_jdown() { return .pfmet_uncorr_jdown(); }
const float &pfmet_uncorr_phi_jdown() { return .pfmet_uncorr_phi_jdown(); }
const float &pfmet_resup() { return .pfmet_resup(); }
const float &pfmet_phi_resup() { return .pfmet_phi_resup(); }
const float &pfmet_resdown() { return .pfmet_resdown(); }
const float &pfmet_phi_resdown() { return .pfmet_phi_resdown(); }
const float &pfmet_rl_resup() { return .pfmet_rl_resup(); }
const float &pfmet_phi_rl_resup() { return .pfmet_phi_rl_resup(); }
const float &pfmet_rl_resdown() { return .pfmet_rl_resdown(); }
const float &mt_met_lep_resup() { return .mt_met_lep_resup(); }
const float &mt_met_lep_rl_resup() { return .mt_met_lep_rl_resup(); }
const float &mt_met_lep_resdown() { return .mt_met_lep_resdown(); }
const float &mt_met_lep_rl_resdown() { return .mt_met_lep_rl_resdown(); }
const float &topnessMod_resup() { return .topnessMod_resup(); }
const float &topnessMod_rl_resup() { return .topnessMod_rl_resup(); }
const float &topnessMod_resdown() { return .topnessMod_resdown(); }
const float &topnessMod_rl_resdown() { return .topnessMod_rl_resdown(); }
const float &mindphi_met_j1_j2_resup() { return .mindphi_met_j1_j2_resup(); }
const float &mindphi_met_j1_j2_rl_resup() { return .mindphi_met_j1_j2_rl_resup(); }
const float &mindphi_met_j1_j2_resdown() { return .mindphi_met_j1_j2_resdown(); }
const float &mindphi_met_j1_j2_rl_resdown() { return .mindphi_met_j1_j2_rl_resdown(); }
const float &MT2_ll_resup() { return .MT2_ll_resup(); }
const float &MT2_ll_resdown() { return .MT2_ll_resdown(); }
const float &genmet_rl() { return .genmet_rl(); }
const float &genmet_phi_rl() { return .genmet_phi_rl(); }
const float &topnessMod_genmet() { return .topnessMod_genmet(); }
const float &topnessMod_rl_genmet() { return .topnessMod_rl_genmet(); }
const float &mt_genmet_lep() { return .mt_genmet_lep(); }
const float &mt_genmet_lep_rl() { return .mt_genmet_lep_rl(); }
const float &mindphi_genmet_j1_j2() { return .mindphi_genmet_j1_j2(); }
const float &mindphi_genmet_j1_j2_rl() { return .mindphi_genmet_j1_j2_rl(); }
const float &MT2_ll_genmet() { return .MT2_ll_genmet(); }
const bool &filt_cscbeamhalo() { return .filt_cscbeamhalo(); }
const bool &filt_cscbeamhalo2015() { return .filt_cscbeamhalo2015(); }
const bool &filt_globaltighthalo2016() { return .filt_globaltighthalo2016(); }
const bool &filt_globalsupertighthalo2016() { return .filt_globalsupertighthalo2016(); }
const bool &filt_ecallaser() { return .filt_ecallaser(); }
const bool &filt_ecaltp() { return .filt_ecaltp(); }
const bool &filt_ecalbadcalib() { return .filt_ecalbadcalib(); }
const bool &filt_eebadsc() { return .filt_eebadsc(); }
const bool &filt_goodvtx() { return .filt_goodvtx(); }
const bool &filt_badevents() { return .filt_badevents(); }
const bool &filt_hbhenoise() { return .filt_hbhenoise(); }
const bool &filt_hbheisonoise() { return .filt_hbheisonoise(); }
const bool &filt_hcallaser() { return .filt_hcallaser(); }
const bool &filt_trkfail() { return .filt_trkfail(); }
const bool &filt_trkPOG() { return .filt_trkPOG(); }
const bool &filt_trkPOG_logerr_tmc() { return .filt_trkPOG_logerr_tmc(); }
const bool &filt_trkPOG_tmc() { return .filt_trkPOG_tmc(); }
const bool &filt_trkPOG_tms() { return .filt_trkPOG_tms(); }
const int &firstGoodVtxIdx() { return .firstGoodVtxIdx(); }
const bool &filt_badChargedCandidateFilter() { return .filt_badChargedCandidateFilter(); }
const bool &filt_badMuonFilter() { return .filt_badMuonFilter(); }
const bool &filt_met() { return .filt_met(); }
const bool &filt_fastsimjets() { return .filt_fastsimjets(); }
const bool &filt_fastsimjets_jup() { return .filt_fastsimjets_jup(); }
const bool &filt_fastsimjets_jdown() { return .filt_fastsimjets_jdown(); }
const bool &filt_jetWithBadMuon() { return .filt_jetWithBadMuon(); }
const bool &filt_jetWithBadMuon_jup() { return .filt_jetWithBadMuon_jup(); }
const bool &filt_jetWithBadMuon_jdown() { return .filt_jetWithBadMuon_jdown(); }
const bool &filt_pfovercalomet() { return .filt_pfovercalomet(); }
const bool &filt_badmuons() { return .filt_badmuons(); }
const bool &filt_duplicatemuons() { return .filt_duplicatemuons(); }
const bool &filt_nobadmuons() { return .filt_nobadmuons(); }
const float &dR_lep_leadb() { return .dR_lep_leadb(); }
const float &dphi_Wlep() { return .dphi_Wlep(); }
const float &MET_over_sqrtHT() { return .MET_over_sqrtHT(); }
const float &MT2_lb_b() { return .MT2_lb_b(); }
const float &MT2_lb_b_mass() { return .MT2_lb_b_mass(); }
const float &MT2_lb_bqq() { return .MT2_lb_bqq(); }
const float &MT2_lb_bqq_mass() { return .MT2_lb_bqq_mass(); }
const float &Mjjj() { return .Mjjj(); }
const float &Mjjj_lep2() { return .Mjjj_lep2(); }
const float &MT2_had() { return .MT2_had(); }
const float &MT2_had_genmet() { return .MT2_had_genmet(); }
const float &MT2_had_jup() { return .MT2_had_jup(); }
const float &MT2_had_jdown() { return .MT2_had_jdown(); }
const float &mht_pt() { return .mht_pt(); }
const float &mht_pt_jup() { return .mht_pt_jup(); }
const float &mht_pt_jdown() { return .mht_pt_jdown(); }
const float &mht_phi() { return .mht_phi(); }
const float &mht_phi_jup() { return .mht_phi_jup(); }
const float &mht_phi_jdown() { return .mht_phi_jdown(); }
const float &pfmet_egclean() { return .pfmet_egclean(); }
const float &pfmet_egclean_phi() { return .pfmet_egclean_phi(); }
const float &pfmet_muegclean() { return .pfmet_muegclean(); }
const float &pfmet_muegclean_phi() { return .pfmet_muegclean_phi(); }
const float &pfmet_muegcleanfix() { return .pfmet_muegcleanfix(); }
const float &pfmet_muegcleanfix_phi() { return .pfmet_muegcleanfix_phi(); }
